 ...
 **************************************************/
static bit32_t keccak_absorb(bit64_t s[25], bit32_t pos, bit32_t r,
                             const bit8_t *in, bit32_t inlen) {
  bit32_t i;

  while (pos + inlen >= r) {
    for (i = pos; i < r; i++) s[i / 8] ^= (bit64_t)*in++ << 8 * (i % 8);
    inlen -= r - pos;
    KeccakF1600_StatePermute(s);
    pos = 0;
  }

  for (i = pos; i < pos + inlen; i++) s[i / 8] ^= (bit64_t)*in++ << 8 * (i % 8);
 ...
 * Returns new position pos in current block
 **************************************************/
static bit32_t keccak_squeeze(bit8_t *out, bit32_t outlen, bit64_t s[25],
                              bit32_t pos, bit32_t r) {
  bit32_t i;
 ...
 *functions
 **************************************************/
static void keccak_absorb_once(bit64_t s[25], bit32_t r, const bit8_t *in,
                               bit32_t inlen, bit8_t p) {
  bit32_t i;
  ...
  for (i = 0; i < inlen; i++) s[i / 8] ^= (bit64_t)in[i] << 8 * (i % 8);

  s[i / 8] ^= (bit64_t)p << 8 * (i % 8);
  s[(r - 1) / 8] ^= 1ULL << 63;
}
 ...
 *              - bit32_t r: rate in bytes (e.g., 168 for SHAKE128)
 **************************************************/
static void keccak_squeezeblocks(bit8_t *out, bit32_t nblocks, bit64_t s[25],
                                 bit32_t r) {
  bit32_t i;
