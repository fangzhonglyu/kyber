                       ...
                       const polyvec *a )
{
  ...
  for ( i = 0; i < KYBER_K; i++ ) {
    for ( j = 0; j < KYBER_N / 4; j++ ) {
      ...
      r[0] = (bit8_t) t[0];
      r[1] = (bit8_t) ( ( t[0] ) >> 8 ) | ( ( t[1] ) << 2 );
      r[2] = (bit8_t) ( ( t[1] ) >> 6 ) | ( ( t[2] ) << 4 );
      r[3] = (bit8_t) ( ( t[2] ) >> 4 ) | ( ( t[3] ) << 6 );
      r[4] = (bit8_t) ( ( t[3] ) >> 2 );
      // for(int i = 0; i < 5; i++){
      //   printf("r[%d]: %x\n", i, (int)r[i]);
      // }
 ...
 *
 * Arguments:   - polyvec *r:       pointer to output vector of
 *polynomials
                         ...
                         const bit8_t a[KYBER_POLYVECCOMPRESSEDBYTES] )
{
  ...
  for ( i = 0; i < KYBER_K; i++ ) {
    for ( j = 0; j < KYBER_N / 8; j++ ) {
      t[0] = ( (bit16_t) a[0] ) | ( (bit16_t) a[1] << 8 );
      t[1] = ( (bit16_t) a[1] >> 3 ) | ( (bit16_t) a[2] << 5 );
      t[2] = ( (bit16_t) a[2] >> 6 ) | ( (bit16_t) a[3] << 2 ) |
             ( (bit16_t) a[4] << 10 );
      t[3] = ( (bit16_t) a[4] >> 1 ) | ( (bit16_t) a[5] << 7 );
      t[4] = ( (bit16_t) a[5] >> 4 ) | ( (bit16_t) a[6] << 4 );
      t[5] = ( (bit16_t) a[6] >> 7 ) | ( (bit16_t) a[7] << 1 ) |
             ( (bit16_t) a[8] << 9 );
      t[6] = ( (bit16_t) a[8] >> 2 ) | ( (bit16_t) a[9] << 6 );
      t[7] = ( (bit16_t) a[9] >> 5 ) | ( (bit16_t) a[10] << 3 );
      a += 11;
  ...
  for ( i = 0; i < KYBER_K; i++ ) {
    for ( j = 0; j < KYBER_N / 4; j++ ) {
      t[0] = ( (bit16_t) a[0] ) | ( (bit16_t) a[1] << 8 );
      t[1] = ( (bit16_t) a[1] >> 2 ) | ( (bit16_t) a[2] << 6 );
      t[2] = ( (bit16_t) a[2] >> 4 ) | ( (bit16_t) a[3] << 4 );
      t[3] = ( (bit16_t) a[3] >> 6 ) | ( (bit16_t) a[4] << 2 );
      a += 5;
 ...
 *
 * Description: Multiply elements of a and b in NTT domain, accumulate
 *into r, and multiply by 2^-16.
 *
