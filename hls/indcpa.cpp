 ...
 *vector
 *              - bit8_t *seed: pointer to output seed to generate matrix
 *A
 *              - const bit8_t *packedpk: pointer to input serialized
 *public key
 **************************************************/
static void unpack_pk(
    polyvec *pk, bit8_t seed[KYBER_SYMBYTES],
 ...
 *(secret key)
 **************************************************/
static void pack_sk( bit8_t r[KYBER_INDCPA_SECRETKEYBYTES], polyvec *sk )
{
 ...
 *(secret key)
 *              - const bit8_t *packedsk: pointer to input serialized
 *secret key
 **************************************************/
static void unpack_sk(
    polyvec *sk, const bit8_t packedsk[KYBER_INDCPA_SECRETKEYBYTES] )
{
 ...
 **************************************************/
static void pack_ciphertext( bit8_t r[KYBER_INDCPA_BYTES], polyvec *b,
                             poly *v )
{
 ...
 *
 * Arguments:   - polyvec *b: pointer to the output vector of polynomials
 *b
 *              - poly *v: pointer to the output polynomial v
 *              - const bit8_t *c: pointer to the input serialized
 *ciphertext
 **************************************************/
 ...
 *              - bit32_t len: requested number of 16-bit integers
 *(uniform mod q)
 *              - const bit8_t *buf: pointer to input buffer (assumed to
                            ...
                            bit32_t buflen )
{
  ...
  while ( ctr < len && pos + 3 <= buflen ) {
    val0 = ( ( buf[pos + 0] >> 0 ) | ( (bit16_t) buf[pos + 1] << 8 ) ) &
           0xFFF;
    val1 = ( ( buf[pos + 1] >> 4 ) | ( (bit16_t) buf[pos + 2] << 4 ) ) &
           0xFFF;
    pos += 3;
 ...
 * Name:        gen_matrix
 *
 * Description: Deterministically generate matrix A (or the transpose of
 *A) from a seed. Entries of the matrix are polynomials that look
...
void gen_matrix( polyvec *a, const bit8_t seed[KYBER_SYMBYTES],
                 bit transposed )
{
  bit32_t   ctr, i, j;
  bit32_t   buflen;
 ...
 *              - const bit8_t *coins: pointer to input random coins used
