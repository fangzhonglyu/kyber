...
// host.cpp
// =======================================================================
// A host for using the hardened Kyber algorithm from the FPGA core
...
int main( int argc, char **argv )
{
  ...
  for ( int rep = 0; rep < REPS; rep++ ) {
    ...
    for ( int i = 0; i < CRYPTO_BYTES; i = i + 4 ) {
      bit32_t key_b_word;
      int     nbytes =
          read( fdr, (void *) &key_b_word, sizeof( key_b_word ) );
        ...
        for ( int i = 0; i < CRYPTO_BYTES; i++ ) {
          if ( key_a[rep][i] == key_b[rep][i] ) {
            printf( ANSI_COLOR_GREEN "%d: %s == %s\n" ANSI_COLOR_RESET, i,
                    key_a[rep][i].to_string( AP_HEX ).c_str(),
                    key_b[rep][i].to_string( AP_HEX ).c_str() );
          }
          else {
